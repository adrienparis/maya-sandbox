...
class Publisher(object):
    ...
    class PathLine():
        ...
        def load(self):
            ...
            self.button = cmds.button(parent=self.layout, label=self.textButton, w=60, bgc=Publisher.LIGHTGREY)
    ...
    def confoAnimEvent(self, *args):
        ...
        # increment and save
        mel.eval("incrementAndSaveScene 1;")
        print("increment and save")
        # store current file
        self.wipRollback = os.path.abspath(cmds.file(q=True, sn=True))
        pubPath = self.wipRollback[:-3] + ".confo" + self.wipRollback[-3:]
        cmds.file(rename=pubPath)
        cmds.file(save=True, type='mayaAscii')

        print("store current file")
        ...
        for e in l:
            try:
                ...
                pass
        print("swap all rig/mod ref to surf ref")
        #publish version
        localPath = self.localPath.path
        relativePath = self.relativePath.path
        ...
        print(versionPath)
        print(relativePath)
        print(fileNameVersion)
        print(fileNamePublish)
        if not os.path.exists(os.path.join(localPath, versionPath)):
            os.makedirs(os.path.join(localPath, versionPath))
        shutil.copy(os.path.join(localPath, relativePath), os.path.join(localPath, versionPath, fileNameVersion))
        shutil.copy(os.path.join(localPath, relativePath), os.path.join(localPath, publishPath, fileNamePublish))
    ...
    def load(self):
        ...
        self.win = cmds.workspaceControl(self.name, ih=100, iw=200, retain=False, floating=True, h=100, w=200)
        ...
        self.localPath.textButton = "Get Project"
        ...
        self.relativePath.textButton = "Refresh"
        ...
        self.checkDefaultBtn = cmds.button(parent=self.defaultActionLay, label="Check", h=20, bgc=Publisher.LIGHTGREY, en=False)
        ...
        cmds.formLayout(self.defaultActionLay, e=True, af=[(self.prepPublishDefaultBtn, "top", margin), (self.prepPublishDefaultBtn, "left", margin),
                                                       ...
                                                       ac=[(self.checkDefaultBtn, "top", 2 * margin, self.prepPublishDefaultBtn),
                                                           (self.publishDefaultBtn, "top", 2 * margin, self.checkDefaultBtn),
        ...
        self.uploadLastVersionAnimBtn = cmds.button(parent=self.animActionLay, label="Upload Last Version", h=20, bgc=Publisher.BLUE, en=False)
