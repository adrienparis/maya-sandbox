...
"""ctrlVisSwitch.py: Just an interface to switch the visibility of ctrls"""
...
@singleton
class SwitchVisibility(Module):
    THEME_BUTTON = Module.COLOR_LIGHTGREY
    THEME_SELECTED = Module.COLOR_BLUE

    class Categorie(Module):
        def __init__(self, parent, name, attrs):
            Module.__init__(self, parent)
            self.name = name
            self.attrs = [[a, False] for a in attrs]

        def setColor(self, attr, v):
            pos = next((i for i, item in enumerate(self.attrs) if item[0] == attr), -1)
            # pos = self.attrs.index(attr)
            self.attrs[pos][1] = v
            color = SwitchVisibility.THEME_SELECTED if self.attrs[pos][1] else SwitchVisibility.THEME_BUTTON
            cmds.control(self.btns[attr], e=True, bgc=color)

        def load(self):
            self.layout = cmds.formLayout(parent=self.parent, bgc=Module.COLOR_DARKGREY)
            self.title = self.attach(cmds.button(p=self.layout, l=self.name, c=Callback(self.runEvent, "switchAll")))
            prev = self.title
            self.btns = {}
            for a in self.attrs:
                prev = self.attach(cmds.button(p=self.layout, l=a[0], c=Callback(self.runEvent, "switch", a[0])), top=prev, left="FORM", right="FORM", margin=(3,3,3,3))
                self.btns[a[0]] = prev
            self.applyAttach()

    class AssetVis(Module):
        def __init__(self, parent, ctrl):
            Module.__init__(self, parent)
            self.ctrl = ctrl
            self.categories = self.getCat(ctrl)

        def getCat(self, name):
            attrs = cmds.listAttr(name)
            i = attrs.index("rotationInterpolation") + 1
            attrs = attrs[i:]
            current = None
            cat = {}
            l = []
            for a in attrs:
                if cmds.getAttr("{}.{}".format(self.ctrl, a), type=True) != "bool":
        ...
        def switch(self, cat, attr):
            pos = next((i for i, item in enumerate(cat.attrs) if item[0] == attr), -1)
            v = not cat.attrs[pos][1]
            cat.setColor(attr, v)
            cmds.setAttr("{}.{}".format(self.ctrl, attr), v)

        def switchAll(self, cat):
            hide = False
            for a, v in cat.attrs:
                if not v:
                    hide = True
                    break
            for a, v in cat.attrs:
                cat.setColor(a, hide)
                cmds.setAttr("{}.{}".format(self.ctrl, a), hide)

        def initButton(self):
            for char, cat in self.categoriesLays.items():
                for a in cat.attrs:
                    v = cmds.getAttr("{}.{}".format(self.ctrl, a[0]))
                    cat.setColor(a[0], v)

        def load(self):
            self.layout = cmds.formLayout(parent=self.parent, bgc=Module.COLOR_GREY)
            self.categoriesLays = {}
            prev = "FORM"
            for cat, attrs in self.categories.items():
                prev = self.attach(SwitchVisibility.Categorie(self.layout, cat, attrs).load(), top=prev, left="FORM", right="FORM", margin=(8,8,3,3))
                prev.eventHandler("switch", self.switch, prev)
                prev.eventHandler("switchAll", self.switchAll, prev)
                self.categoriesLays[cat] = prev
            self.initButton()
    ...
    def load(self):
        ...
        self.attach(SwitchVisibility.AssetVis(self, "C1:c_visibility").load(), top="FORM", bottom="FORM", left="FORM", right="FORM", margin=(0,0,0,0))
