...
# joint Ori Zero
def createInterJoint(jointName, interName, value, side, type_):
    cmds.select(jointName)
    size = cmds.getAttr(jointName + ".radius")
    cmds.joint(name=interName, radius=size)
    p = cmds.listRelatives(jointName, p=True)
    if p is None or p == []:
        return
    try:
        cmds.parent(interName, jointName)
    except:
        print("lol " + jointName)
    if type_ == "Ori":
        if side == "L":
            cmds.setAttr(interName + ".tz", 0.001)
        elif side == "R":
            cmds.setAttr(interName + ".tz", -0.001)
    elif type_ == "0":
        if side == "L":
            ...
            cmds.setAttr(interName + ".tz", 0.001)
    cmds.parent(interName, p)
    cmds.pointConstraint( jointName, interName, mo=True)
    cmds.setAttr(interName + ".visibility", False)
    nameMd = interName.replace("sk_","md_")
    cmds.createNode("multiplyDivide", name=nameMd)

    cmds.connectAttr(jointName + ".rotate", nameMd + ".input1")
    cmds.connectAttr(nameMd + ".output", interName + ".rotate")

    v = 0.5 if type_ == "0" else 0


    cmds.setAttr(nameMd + ".input2X", v)
    cmds.setAttr(nameMd + ".input2Y", value)
    cmds.setAttr(nameMd + ".input2Z", v)

def createOri(joint, value):
    #define size
    s = "L" if joint.endswith("_L") else "R" if joint.endswith("_R") else ""
    #define index to insert in name
    l = len(joint) - 2 if (joint.endswith("_L") or joint.endswith("_R")) else len(joint)
    nameOri = joint[:l] + 'Ori' + joint[l:]
    createInterJoint(joint, nameOri, value, s, "Ori")

def create0(joint, value):
    #define size
    s = "L" if joint.endswith("_L") else "R" if joint.endswith("_R") else ""
    #define index to insert in name
    l = len(joint) - 2 if joint.endswith("_L") or joint.endswith("_R") else len(joint)
    ...
    createInterJoint(joint, name0, value, s, "0")

def generateInter(type_, ff):
    sl = cmds.ls(sl=True)
    ...
    for ctrl in cmds.ls(sl=True):
        ...
        if cmds.objExists(skin):
            ...
            if cmds.objExists(skin0):
                ...
                cmds.connectAttr(pSkin + ".scale", node + ".input3D[1]")
...
def miniToolRigWin():
    ...
    # TMP creation
    fLayTMP = cmds.formLayout(p=win, bgc=[0.35, 0.4, 0.5])
    ...
    fLayOriZero = cmds.formLayout(p=win, bgc=[0.35, 0.4, 0.5])
    ffOri = cmds.floatField(p=fLayOriZero, pre=3, v=0)
    bOri = cmds.button(p=fLayOriZero, label="Create Ori", en=True, c=Callback(generateInter, "Ori", ffOri))
    ffZero = cmds.floatField(p=fLayOriZero, pre=3, v=0.5)
    bZero = cmds.button(p=fLayOriZero, label="Create 0", en=True, c=Callback(generateInter, "0", ffZero))
    ...
    cmds.button(parent=win, label="connect CTRLs' scales (fingers)", c=Callback_miniToolRig(connectScaleCtrl))
