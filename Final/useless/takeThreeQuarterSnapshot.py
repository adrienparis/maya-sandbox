...
def take_screenShot(name, width=1920, height=1080, frame=100):
    ...
    cmds.playblast(st=frame, et=frame + 16, v=False, fmt="movie", orn=False, cf=img_path, wh=[width,height], p=100)
...
switch_light(True)
cmds.lookThru('CAM')
take_screenShot(In("cam"), frame=v)
cmds.lookThru('front')
take_screenShot(In("front"), frame=v)
cmds.lookThru('side')
take_screenShot(In("side"), frame=v)